...
try {
  ...
  logResult('AuthUtils.ts exists with required functions', 
    hasWithAuth && hasAuthenticateRequest && hasValidateOrganizationAccess && hasSecurityHeaders,
    `withAuth: ${hasWithAuth}, authenticateRequest: ${hasAuthenticateRequest}, validateOrgAccess: ${hasValidateOrganizationAccess}, securityHeaders: ${hasSecurityHeaders}`
  );
} catch (error) {
...
try {
  ...
  logResult('ValidationSchemas.ts exists with comprehensive Zod validation', 
    hasCreateMaterialSchema && hasGetMaterialsQuerySchema && hasCreateOrganizationSchema && hasSanitization && hasXSSProtection,
    `Schemas present: ${hasCreateMaterialSchema && hasGetMaterialsQuerySchema && hasCreateOrganizationSchema}, XSS protection: ${hasXSSProtection}`
  );
} catch (error) {
...
try {
  ...
  logResult('Materials API route implements security protections', 
    usesWithAuth && usesValidation && usesSecurityHeaders && checksOrganization && hasRateLimit,
    `withAuth: ${usesWithAuth}, validation: ${usesValidation}, headers: ${usesSecurityHeaders}, orgCheck: ${checksOrganization}, rateLimit: ${hasRateLimit}`
  );
} catch (error) {
...
try {
  ...
  logResult('Organizations API route implements security protections', 
    usesWithAuth && usesValidation && usesSecurityHeaders && checksOrganization && hasRateLimit,
    `withAuth: ${usesWithAuth}, validation: ${usesValidation}, headers: ${usesSecurityHeaders}, orgCheck: ${checksOrganization}, rateLimit: ${hasRateLimit}`
  );
} catch (error) {
...
try {
  ...
  logResult('Middleware protects API routes and dashboard', 
    usesClerkMiddleware && protectsApiRoutes && hasPublicApiRoutes && protectsDashboard && hasOrgRedirect,
    `Clerk: ${usesClerkMiddleware}, API protection: ${protectsApiRoutes}, public routes: ${hasPublicApiRoutes}, dashboard: ${protectsDashboard}, org redirect: ${hasOrgRedirect}`
  );
} catch (error) {
...
try {
  ...
  logResult('Next.js security headers are properly configured', 
    hasHeaders && hasXFrameOptions && hasContentTypeOptions && hasCSP && hasHSTS && hasApiCacheHeaders,
    `Headers function: ${hasHeaders}, XFO: ${hasXFrameOptions}, XCTO: ${hasContentTypeOptions}, CSP: ${hasCSP}, HSTS: ${hasHSTS}, API cache: ${hasApiCacheHeaders}`
  );
...
try {
  ...
  logResult('Database schema includes security fields', 
    hasOrganizationId && hasTrainerId && hasTimestamps,
    `organizationId: ${hasOrganizationId}, trainerId: ${hasTrainerId}, timestamps: ${hasTimestamps}`
  );
} catch (error) {
...
try {
  ...
  logResult('Environment variables are properly configured', 
    hasClerkConfig && hasDatabaseConfig && hasStripeConfig,
    `Clerk: ${hasClerkConfig}, Database: ${hasDatabaseConfig}, Stripe: ${hasStripeConfig}`
  );
} catch (error) {
