...
test.describe('Cross-Browser Compatibility', () => {
  ...
  test.describe('@browser CSS and Styling Compatibility', () => {
    test('Modern CSS features should work across browsers', async ({ page, browserName }) => {
      ...
      if (browserName !== 'webkit' || parseFloat(process.env.WEBKIT_VERSION || '14') >= 14) {
    ...
    test('Modern JavaScript features should work', async ({ page, browserName }) => {
      ...
      const jsFeatures = await page.evaluate(() => {
        const features = {
          ...
          arrow: (() => true)(),
          destructuring: (() => { try { const {a} = {a: 1}; return a === 1; } catch { return false; } })(),
          templateLiterals: (() => { try { return `test${1}` === 'test1'; } catch { return false; } })(),
          fetch: typeof fetch === 'function',
          promise: typeof Promise === 'function',
          classes: (() => { try { class Test {} return true; } catch { return false; } })(),
          modules: 'import' in window,
          const: (() => { try { const a = 1; return true; } catch { return false; } })(),
          let: (() => { try { let a = 1; return true; } catch { return false; } })()
        };
